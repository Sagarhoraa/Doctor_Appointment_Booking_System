...
const Appointment = () => {
  ...
  const getAvailebleSlots = async () => {
    ...
    for(let i=0 ; i < 7 ; i++){
      //getting the date also with index 
      let currentDate = new Date(today)
      currentDate.setDate(today.getDate()+i)

      //set the time and date also with index
  ...
  return docInfo &&(
    <div>
      {/* ----Doctors ko details heru----- */}
      <div className='flex flex-col sm:flex-row gap-4'>
        <div>
          <img className='bg-primary w-full sm:max-w-72 rouned-lg' src={docInfo?.image} alt="" />
        </div>
          <div className='flex-1 border border-gray-400 rounded-lg p-8  py-7 bg-white mx-2
           sm:mx-0 mt-[-80px] sm:mt-0'>
            {/* ---------------doc info for name degree and expreience----------- */}
